...
h=Label(new,text="number and range")from tkinter import*
from tkinter.ttk import*
new=Tk()
new.title("MULTIPILCATION TABLE GENERATOR")
title=Label(new,text="Multiplication table")
...
f=IntVar
g=Radiobutton(new,text="10",variable=f,value=10)
g1=Radiobutton(new,text="20",variable=f,value=20)
g2=Radiobutton(new,text="30",variable=f,value=30)
g.grid(row=2,column=3)
g1.grid(row=3,column=3)
g2.grid(row=4,column=3)
def generate():
    Tables=""
    for i in range(f.get()+1):
        Tables+=str(z.get())+"x"+str(f)+"="+str(z*f)+"\n"
